...
module aptos_framework::multisig_account {
    ...
    public fun can_execute(
        owner: address, multisig_account: address, sequence_number: u64): bool acquires MultisigAccount {
        let multisig_account_resource = borrow_global<MultisigAccount>(multisig_account);
        assert_is_owner_addr(owner, multisig_account_resource);
        assert!(
            sequence_number > 0 && sequence_number < multisig_account_resource.next_sequence_number,
            error::invalid_argument(EINVALID_SEQUENCE_NUMBER),
        );
        ...
        let voted = simple_map::contains_key(&transaction.votes, &owner);
        let vote = if (voted) { *simple_map::borrow(&transaction.votes, &owner) } else { false };
        if (!voted || !vote) {
    ...
    fun validate_multisig_transaction(
        ...
        // Transaction execution implicitly votes for approval.
        let voted = simple_map::contains_key(&transaction.votes, &owner_addr);
        let vote = if (voted) { *simple_map::borrow(&transaction.votes, &owner_addr) } else { false };
        if(!voted || !vote) {
            num_approvals = num_approvals + 1;
        };
        assert!(
    ...
    fun successful_transaction_execution_cleanup(
        ...
        let sequence_number = multisig_account_resource.last_executed_sequence_number + 1;
        let transaction = table::borrow(&multisig_account_resource.transactions, sequence_number);
        let voted = simple_map::contains_key(&transaction.votes, &executor);
        let vote = if (voted) { *simple_map::borrow(&transaction.votes, &executor) } else { false };
    ...
    fun assert_is_owner(owner: &signer, multisig_account: &MultisigAccount) {
        assert_is_owner_addr(address_of(owner), multisig_account);
    }

    fun assert_is_owner_addr(owner: address, multisig_account: &MultisigAccount) {
        assert!(
            vector::contains(&multisig_account.owners, &owner),
