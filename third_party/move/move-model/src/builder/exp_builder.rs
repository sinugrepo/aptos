...
use crate::{
    ...
    ty::{
        Constraint, PrimitiveType, ReceiverFunctionInstance, ReferenceKind, Substitution, Type,
...
impl<'env, 'translator, 'module_translator> ExpTranslator<'env, 'translator, 'module_translator> {
    ...
    fn fresh_type_var_constr(&mut self, loc: Loc, order: WideningOrder, ctr: Constraint) -> Type {
        ...
        self.add_constraint(&loc, &Type::Var(idx), order, ctr)
...
impl<'env, 'translator, 'module_translator> ExpTranslator<'env, 'translator, 'module_translator> {
    ...
    fn translate_fun_call(
        ...
        if let Some(value) =
            self.translate_fun_call_special_cases(expected_type, loc, kind, maccess, generics, args)
        {
            return value;
        }
        ...
        let result =
            self.translate_call(loc, kind, &module_name, name, generics, args, expected_type);
    ...
    fn translate_update_field(
        ...
        ) = &args[1].value
        {
            ...
            if let Type::Struct(mid, sid, inst) = self.subs.specialize(expected_type) {
                ...
                if let Err(e) =
                    self.add_constraint(loc, expected_type, WideningOrder::RightToLeft, constraint)
                {
        ...
        for cand in &cands {
            ...
            if let AnyFunEntry::SpecOrBuiltin(sbf) = cand {
                if let Err(err) = self.add_constraints(
                    ...
                    outruled.push((cand, err.specific_loc(), err.message(self, &tctx)));
                    continue;
                }
            }
            ...
            for (i, arg_ty) in arg_types.iter().enumerate() {
                ...
                if let Err(err) = subs.unify(
                    ...
                    outruled.push((
                        cand,
                        arg_loc,
                        format!("{} for argument {}", err.message(self, &tctx), i + 1),
                    ));
                    success = false;
                    break;
                }
