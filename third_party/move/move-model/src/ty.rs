...
impl Substitution {
    ...
    ///
    /// If spec-variance is enabled (i.e. the type appears in the specification language):
    ///
    /// - References are dropped (i.e. &T and T are compatible)
    /// - All integer types are compatible.
    /// - With the joint effect of the above, if (P, Q) is compatible under spec-variance,
    ///   (&P, Q), (P, &Q), and (&P, &Q) are all compatible under co-variance.
    /// - If in two tuples (P1, P2, ..., Pn) and (Q1, Q2, ..., Qn), all (Pi, Qi) pairs are
    ///   compatible under spec-variance, then the two tuples are compatible under
    ///   spec-variance.
    ///
    /// In the programming language, the following variance rules are applied:
    ///
    /// - `&mut T` can be widened to `&T`
    ///
            ...
            (Type::Struct(m1, s1, ts1), Type::Struct(m2, s2, ts2)) => {
                if m1 == m2 && s1 == s2 {
                    ...
                    return Ok(Type::Struct(
                        *m1,
                        *s1,
                        self.unify_vec(context, variance, order, None, ts1, ts2, "struct field")?,
                    ));
                }
            },
    ...
    pub fn message(
        ...
        match self {
            ...
            TypeUnificationError::ArityMismatch(item, a1, a2) => {
                format!("{} count mismatch (found {} expected {})", item, a1, a2)
            },
